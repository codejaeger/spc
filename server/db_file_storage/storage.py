...
class DatabaseFileStorage(Storage):
    ...
    def _getme(self, encoded_bytes):
        # file_buffer = base64.b64decode(encoded_bytes)
        return ContentFile(encoded_bytes)
    ...
    def _open(self, name,passw, mode='rb'):
        ...
        # _fill = base64.b64decode(encoded_bytes)
        # _fill = 

        # _fill = base64.b64decode(encoded_bytes)
        # _fin = io.BytesIO(_fill)
        # _fdec = io.BytesIO()
        # pyAesCrypt.decryptStream(_fin,_fdec,passw,bf,len(_fin.getvalue()))
        # e(_fdec.getvalue())
        # _fill = base64.b64decode(encoded_bytes)
        # _fill = str.encode(encoded_bytes)
        # _fin = io.BytesIO(_fill)
        # k = pyDes.des(b'12345678',pyDes.CBC,b'\0\0\0\0\0\0\0\0',pad=None,padmode=pyDes.PAD_PKCS5)
        # d = k.decrypt(_fill)
        # pyAesCrypt.decryptStream(_fin,_fdec,passw,bf,len(_fin.getvalue()))
        

        # _file = self._getme(_fin.getvalue())
        # _file = self._getme(_fdec.getvalue())
    ...
    def _openlnx(self, name, mode='rb'):
        assert mode[0] in 'rwab'
        # bf = 64 * 1024
        if os.sep != '/':  # Windows fix (see a6d4707) # pragma: no cover
            name = name.replace('/', os.sep)
    ...
    def _save(self, name, content):
        ...
        # raise ValidationError(content.open('rb').read())
        # enc = des("DESCRYPT", CBC, "12345678" , pad=None , padmode =PAD_PKCS5)
        # plt = enc.encrypt(content.open('rb').read())
        # content.open('wb').write(plt)
        # content.close()
        # raise ValidationError(content.open('rb').read())
