		...
		if ans == 'Y':
			...
			with open(oldenc,'wb') as fw:
				with open(text_file, 'rb') as fr:
					l = pickle.load(fr)
					pickle.dump(l,fw)
		a = getpass.getpass("Schema used: ")
		if a == 'AES':
			pas = getpass.getpass("Passphrase: ")
			with open(text_file,'wb') as fw:
				pickle.dump(['AES',pas],fw)
		elif a == 'DES':
			pas = getpass.getpass("Passphrase(8 letters): ")
			with open(text_file,'wb') as fw:
				pickle.dump(['DES',pas],fw)
		elif a == 'blowfish':
			pas = getpass.getpass("Passphrase(4-64 letters): ")
			iv = b'\xf9\x11\xeb\x8a\x871\x98\x89'
			with open(text_file,'wb') as fw:
				pickle.dump(['blowfish',pas,iv],fw)
		else:
			print("Wrong Schema")
